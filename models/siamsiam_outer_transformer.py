...
class SiamSiamTransformer(pl.LightningModule):
    """
        This class implements the SSL method, described in SiamSiam
    ...
    def common_step(self, batch, mode):
        ...
        # Send unmasked feats through transformer, but without gradient updates
        with torch.no_grad():
            transformed_feats_unmasked = self.transformer(feats)  # outputs of size (batch, outer, feat)
            aggregated_feats_unmasked = self.aggregator(transformed_feats_unmasked)  # output of size (batch, feat)
        ...
        loss = self.reconstruction_loss(aggregated_feats_unmasked, projected_feats)
